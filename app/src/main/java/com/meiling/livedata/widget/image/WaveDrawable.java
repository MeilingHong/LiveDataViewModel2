package com.meiling.livedata.widget.image;
...
public class WaveDrawable extends Drawable implements Animatable, ValueAnimator.AnimatorUpdateListener {
    /*
    调用示例：
            waveDrawable = new WaveDrawable(getContext(), R.drawable.bg_round_0_update_progress_cover);
        waveDrawable.setIndeterminate(false);
//        waveDrawable.setWaveLength(3);// 似乎没啥太特别的变化【应该是影响波浪的形状】
//        waveDrawable.setWaveSpeed(2);// 设置波动速度【值越大，则越快】
//        waveDrawable.setWaveAmplitude(1);// 设置波动的幅度【值越大，则幅度越大】
        hintIcon.setImageDrawable(waveDrawable);
     */
    ...
    private static ColorFilter sGrayFilter = new ColorMatrixColorFilter(new float[]{
            1, 0, 0, 0, 1,            //  R 红色         第五列决定了对应颜色的偏移量【值越大，偏移越多，但仍需要在范围内】
            0, 1, 0, 0, 1,            //  G 绿色
            0, 0, 1, 0, 1,            //  B 蓝色
            0, 0, 0, 1, 0              //  A 透明
    });//这里50 对应的值越大，则颜色越白，
    private ColorFilter mCurFilter = new ColorMatrixColorFilter(new float[]{
            1, 0, 0, 0, 0,            //  R 红色         第五列决定了对应颜色的偏移量【值越大，偏移越多，但仍需要在范围内】
            0, 1, 0, 0, 0,            //  G 绿色
            0, 0, 1, 0, 0,            //  B 蓝色
            0, 0, 0, 0, 0              //  A 透明
    });//这里50 对应的值越大，则颜色越白，
    ...
    private void init(Drawable drawable) {
        ...
        mPaint.setColor(Color.parseColor("#3296fa"));
    ...
    public void draw(Canvas canvas) {
        mDrawable.setColorFilter(sGrayFilter);// 先显示的图像【通过矩阵转换后的图像】
        mDrawable.draw(canvas);
        mDrawable.setColorFilter(mCurFilter);// 后显示的图像【当mCurFilter==null 时，显示的是原图像】
    ...
    private void updateMask(int width, int length, int height) {
        ...
        p.setColor(Color.parseColor("#3296fa"));
